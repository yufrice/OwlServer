...
login user = do
  ...
  case res of
    ...
    Just r -> do
      hash <- liftIO $ C.hashPasswordUsingPolicy C.slowerBcryptHashingPolicy $ user ^. userPassword
      case hash of
        Nothing -> throwError $ err401
        Just h -> do
          case C.validatePassword h $ (entityVal r) ^. userPassword of
            False -> throwError $ err401
            True -> do
              token <- liftIO $ makeToken 5
              time <- liftIO $ DT.getCurrentTime
              session <- return $ Session token time
              runDB $ insert session
              return
                $ addHeader token
                $ addHeader "Bearer"
                $ addHeader 3600
                $ addHeader "not implemented"
                $ NoContent
