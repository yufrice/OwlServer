module Lib.VectorSimSpec (spec) where
...
testVector1 =  fromList [x | x <- [1..1000]]
...
spec = do 
  describe "Dot product" $ do
    it "10dim" $
      dotProduct testVector0 testVector0 `shouldBe` 1540
    it "1000dim" $
      dotProduct testVector1 testVector1 `shouldBe` 333833500
  describe "L2 Norm" $ do
    it "10dim" $
      l2Norm testVector0 `shouldBe`  2 * sqrt 385
    it "1000dim" $
      l2Norm testVector1 `shouldBe`  10 * sqrt 3338335
  describe "Similarity" $ do
    it "Near" $ do
      similarity testNormalize0Vector testNormalize0Vector `shouldBe` 1
      similarity testNormalize1Vector testNormalize1Vector `shouldSatisfy` liftM2 (&&) (1 + epsilonDouble >=) (1 - epsilonDouble <=)